...
export default function FileViewer({ file, open, onClose }: FileViewerProps) {
  ...
  const isDOCX = fileExtension === 'docx' || fileExtension === 'doc' || file.url?.includes('.docx') || file.url?.includes('.doc');
  ...
  const downloadFile = () => {
    ...
    link.href = file.url;
    link.download = file.label || 'document';
    link.target = '_blank';
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent 
        className={`${fullscreen ? 'max-w-[100vw] h-[100vh] w-[100vw]' : 'max-w-6xl max-h-[90vh]'} p-0`}
        dir="rtl"
      >
        ...
        <div className={`relative ${fullscreen ? 'h-[calc(100vh-73px)]' : 'h-[calc(90vh-73px)]'} bg-slate-100`}>
          {loading && !loadError && (
          ...
          {loadError ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center space-y-4 p-8">
                <p className="text-slate-600 text-lg">לא ניתן להציג את המסמך בחלון זה</p>
                <p className="text-slate-500 text-sm">פתח את המסמך בחלון חדש או הורד אותו</p>
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center space-y-4 p-8">
                <p className="text-slate-600 text-lg">לא ניתן להציג את סוג הקובץ הזה</p>
                <div className="flex gap-3 justify-center">
