...
const PDFContent: React.FC<PDFPreviewProps> = ({ pages, settings, declarationData }) => {
  ...
  return (
    ...
    >
      {settings.showTable && (
        <TablePreview pages={sortedPages} />
      )}
      ...
      {settings.showPages && sortedPages.map((page) => (
        <ItemPreview key={page.id} page={page} />
      ))}
    </Document>
  );
};
