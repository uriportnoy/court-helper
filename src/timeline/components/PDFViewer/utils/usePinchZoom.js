...
const usePinchZoom = ({
  ...
  const handleMouseDown = useCallback(
    (e) => {
      if (scale > initialScale && e.button === 0) {
        // Left mouse button
    ...
    [scale, panPosition, initialScale, containerRef],
  );
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (containerRef.current) {
      containerRef.current.style.cursor =
        scale > initialScale ? "grab" : "default";
    }
  }, [scale, initialScale, containerRef]);
  ...
  const handleTouchStart = useCallback(
    ...
    [scale, panPosition, initialScale],
  );
  ...
  const handleTouchMove = useCallback(
    (e) => {
      if (e.touches.length === 2 && lastTouchRef.current) {
        ...
        const newScale = Math.max(
          minScale,
          Math.min(maxScale, lastTouchRef.current.scale * scaleFactor),
        );
